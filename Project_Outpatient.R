...
# Libs
#----------------------------------------------------------------------------------------
mypackages = c("xgboost", "tidyverse","magrittr","stringr","Boruta","caret","glmnet","e1071","doParallel","foreach")   
...
#*********************
charges_2011 = read.csv("outpatient/outpatient_charges_2011.csv") %>% mutate(charge_year=2012)
charges_2012 = read.csv("outpatient/outpatient_charges_2012.csv") %>% mutate(charge_year=2012)
charges_2013 = read.csv("outpatient/outpatient_charges_2013.csv") %>% mutate(charge_year=2013)
charges_2014 = read.csv("outpatient/outpatient_charges_2014.csv") %>% mutate(charge_year=2014)
charges_2015 = read.csv("outpatient/outpatient_charges_2015.csv") %>% mutate(charge_year=2015)
charges = rbind(charges_2011,charges_2012, charges_2013, charges_2014, charges_2015) %>% mutate(charge_year = as.factor(charge_year))

hospitals = read.csv("outpatient/cms_medicare_hospital_general_info.csv")

race = read.csv("outpatient/race.csv") %>% select(state, state_prct_minority)
...
charges %<>% mutate(drg_num=str_sub(str_trim(apc), 1, 4),
                    drg_num = as.numeric(drg_num))

charges %<>% mutate(drg_num=str_sub(str_trim(apc), 1, 4),
                    drg_num = as.numeric(drg_num))
colnames(charges)[colnames(charges)=="apc"] <- "drg_definition"
...
mdc = combined %>% filter(drg_num >= 12 )
